...
This guide deploys Sente to **Vercel (frontend) + Railway (backend + Postgres)**
for ~$0–5/month. The whole flow takes about 30 minutes the first time.
...
2. Railway provisions a Postgres instance and adds `DATABASE_URL` to your
   service's environment automatically. **Don't override it.**

### 1c. Set the remaining env vars
...
`CORS_ORIGINS` won't be known until step 2 — leave a placeholder, you'll
update it after the frontend is deployed.

### 1d. Generate a public domain
...
running 24/7 with low traffic uses ~$3–4/month. Postgres is another ~$1.
